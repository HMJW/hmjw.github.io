...
这里就用到了变分推断的思想：用一个比较简单的分布$Q(z|x)$去逼近真实的分布$P(x|z)$，也就是最小化两者之间的距离。我们用KL距离来衡量这种关系，根据定义得到如下公式:
$$KL\[Q(z|x)\||P(x|z)] = \sum_{z}Q(z|x)log\frac{Q(z|x)}{P(z|x)} \\ = E_{z}\[logQ(z|x) - logP(z|x)\]$$
根据贝叶斯公式：
$$KL\[Q(z|x)\||P(x|z)] = E_{z}\[logQ(z|x) - log\frac{P(x|c)P(z)}{P(x)}\] \\ = E_{z}\[logQ(z|x) - logP(x|z)-logP(z)\]+logP(x)$$
注意到$P(x)$和$z$无关，所以可以拿到期望外面，所以又有:
$$logP(x) = KL\[Q(z|x)\||P(x|z)] - E_{z}\[logQ(z|x) - logP(x|z)-logP(z)\] \\ = 
KL\[Q(z|x)\||P(x|z)] + E_{z}\[logP(x|z)- (logQ(z|x) - logP(z))\] \\ = 
KL\[Q(z|x)\||P(x|z)] + E_{z}\[logP(x|z)\]- E_{z}\[(logQ(z|x) - logP(z))\] \\ =
KL\[Q(z|x)\||P(x|z)] + E_{z}\[logP(x|z)\]- KL(Q(z|x) \|| P(z))$$
